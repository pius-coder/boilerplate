...
save_mode: false # -s: Save outputs to .codex/output/apex/
...
{task_id} will be generated by the setup script:
  - Script auto-detects next available number in .codex/output/apex/
...
1. **Search for matching task:**
   ...
   ```bash
   ls .codex/output/apex/ | grep "^{resume_task}"
   ```
...
- Creates `.codex/output/apex/{task_id}/` directory
