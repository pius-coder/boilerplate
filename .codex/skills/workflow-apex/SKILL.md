...
- `-s` (save): Save outputs to `.codex/output/apex/`
...
| `-s` | `--save` | Save mode: output each step to `.codex/output/apex/` |
...
All outputs saved to PROJECT directory (where Codex Code is running):
```

.codex/output/apex/{task-id}/
...
1. Locate the task folder in `.codex/output/apex/`
...
| `{save_mode}`           | boolean | Save outputs to .codex/output/apex/                   |
